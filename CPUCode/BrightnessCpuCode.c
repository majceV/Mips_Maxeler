...
void brightness_cpu(unsigned char* bitmapBits, int width, int height)
  {
    ...
    {
    	if(bitmapBits[i] + factor >255)
    		bitmapBits[i] = 255;
    	else if(bitmapBits[i] + factor <0)
    		bitmapBits[i] = 0;
    	else bitmapBits[i] = bitmapBits[i] + factor;
    //	 bitmapBits[i] = bitmapBits[i] + factor < 255 ? bitmapBits[i] + factor : 255;
    }

    FILE* out_cpu = fopen("cpu_result.bmp", "wb");

    write_BITMAPFILEHEADER(out_cpu, width, height);

    write_BITMAPINFOHEADER(out_cpu, width, height);

    fwrite(bitmapBits, 1, width*height*3, out_cpu);
...
int main(void)
{
	...
	unsigned char* bitmapBits_cpu;

	extractPixels("lenny.bmp", &bitmapBits, &width, &height);

	bitmapBits_cpu = (char*)malloc(width*height*3*sizeof(char));
	for(int i = 0; i<  width*height*3; i++)
	{
		bitmapBits_cpu[i] = bitmapBits[i];
	}
	...
	FILE* out = fopen("max_result.bmp", "wb");
